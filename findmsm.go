...
func parseFindMsmFlags(flags *findMsmFlags) (
	filter goatapi.MeasurementFilter,
	...
	if flags.filterStatus != "" {
		...
		for _, item := range strings.Split(strings.ToLower(flags.filterStatus), ",") {
			...
			case "any":
			case "spe":
				fs = append(fs, goatapi.MeasurementStatusSpecified)
			case "sch":
				fs = append(fs, goatapi.MeasurementStatusScheduled)
			case "ong":
				fs = append(fs, goatapi.MeasurementStatusOngoing)
			case "stp":
				fs = append(fs, goatapi.MeasurementStatusStopped)
			case "nos":
				fs = append(fs, goatapi.MeasurementStatusNoSuitableProbes)
			case "den":
				fs = append(fs, goatapi.MeasurementStatusDenied)
			default:
