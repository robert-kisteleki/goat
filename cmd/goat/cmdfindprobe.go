...
func parseFindProbeFlags(flags *findProbeFlags) (
	...
	if flags.sort != "" {
		if goat.ValidProbeListSortOrder(flags.sort) {
	...
	case "N":
		filter.FilterStatus(goat.ProbeStatusNeverConnected)
	case "C":
...
func parseFindProbeArgs(args []string) *findProbeFlags {
	...
	flagsFindProbe.StringVar(&flags.sort, "sort", "-id", "Result ordering: "+strings.Join(goat.ProbeListSortOrders, ","))
